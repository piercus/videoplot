...
class Videoplot {
	...
	*/
	constructor({w, h, fps, duration, logger}) {
		this.w = w;
		this.h = h;
		this.fps = fps;
	...
	*/
	...
	drawVideoChart(opts) {
		...
		for (let i = 0; i < frames - 1; i++) {
			if (this.logger) {
				this.logger.debug('ChartJs before creation');
			}
			const chart = new ChartJs(w, h);
			if (this.logger) {
				this.logger.debug('ChartJs created');
			}
			const optsI = getOpts({
				wScale,
				frames,
			...
			const promise = Promise.resolve(optsI)
				.then(optsI => {
					if (this.logger) {
						this.logger.debug('draw video Chart: launch chart creation ', index);
					}
					return chart.drawChart(optsI);
				})
				.then(res => {
					if (this.logger) {
						this.logger.debug('draw video Chart done', index);
					}
					return res;
				})
				.catch(((optsI, err) => {
		...
		if (this.logger) {
			this.logger.debug('draw video Chart: launch chart creation');
		}
	...
	*/
	...
	getPngStream() {
		...
		if (this.logger) {
			this.logger.debug('getPngStream', this.charts.length);
		}
		return Promise.all(this.charts.map((ch, index) => {
			...
			return ch.writeImageToFile('image/png', image).then(res => {
				if (this.logger) {
			...
			if (this.logger) {
				this.logger.debug('all images written');
			}
